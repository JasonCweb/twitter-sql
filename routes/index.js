...
router.post('/tweets', function (req, res, next) {
  client.query('SELECT name FROM users WHERE name = $1', [req.body.name],
    function (err, result) {
      if (err) return next(err);
      if (result.rows[0] == undefined) {
        client.query('INSERT INTO users (name, picture_url) VALUES ($1, $2)', [req.body.name, 'http://i.imgur.com/XDjBjfu.jpg']);
      }
      client.query('INSERT INTO tweets (user_id, content) SELECT id, $1 FROM users WHERE name = $2', [req.body.content, req.body.name]);
      res.redirect('/');
    });
